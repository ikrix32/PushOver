...
namespace UnityEditor.XCodeEditor 
{
	public class XCPlist
	{
		...
		const string PlistStringContent = "stringcontent";
		...
		public void Process(Hashtable plist)
		{
			...
			foreach( DictionaryEntry entry in plist)
			{
				if (entry.Value is string) {
					this.AddPlistItems((string)entry.Key, ((string)entry.Value).Replace("${PRODUCT_NAME}", PlayerSettings.productName), dict);
				} else {
